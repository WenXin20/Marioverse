...
public class IdMapMixin {
    ...
    private static boolean addBlockEntry(List instance, Object entry, Operation<Boolean> original) {
        if (entry instanceof BlockEntry blockEntry) {
            ...
            if (blockEntry.id().getNamespace().equals("minecraft") && blockEntry.id().getName().equals("dandelion"))
                original.call(instance, new BlockEntry(new NamespacedId(Marioverse.MOD_ID, BlockRegistry.DANGO_BLOSSOM.getId().getPath()), Collections.emptyMap()));
        }
