...
public class HexColorShapelessRecipeTransferHandler implements SimpleTransferHandler {
    private final CategoryIdentifier<DefaultCraftingDisplay<?>> categoryIdentifier;

    public HexColorShapelessRecipeTransferHandler(CategoryIdentifier<DefaultCraftingDisplay<?>> categoryIdentifier) {
    ...
    public List<InputIngredient<ItemStack>> getInputsIndexed(TransferHandler.Context context) {
        if (!(context.getDisplay() instanceof HexColorShapelessCraftingDisplay display))
            return SimpleTransferHandler.super.getInputsIndexed(context);

        RecipeHolder<HexColorShapelessRecipe> recipeHolder = display.getOptionalRecipe().orElse(null);
