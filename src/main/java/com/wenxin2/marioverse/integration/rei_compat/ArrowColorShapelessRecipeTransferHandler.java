...
public class ArrowColorShapelessRecipeTransferHandler implements SimpleTransferHandler {
    private final CategoryIdentifier<DefaultCraftingDisplay<?>> categoryIdentifier;

    public ArrowColorShapelessRecipeTransferHandler(CategoryIdentifier<DefaultCraftingDisplay<?>> categoryIdentifier) {
    ...
    public List<InputIngredient<ItemStack>> getInputsIndexed(TransferHandler.Context context) {
        if (!(context.getDisplay() instanceof ArrowColorShapelessCraftingDisplay display))
            return SimpleTransferHandler.super.getInputsIndexed(context);

        RecipeHolder<ArrowColorShapelessRecipe> recipeHolder = display.getOptionalRecipe().orElse(null);
