...
import com.wenxin2.marioverse.client.models.entities.PorcupufferModel;
...
public class PorcupufferRenderer extends GeoEntityRenderer<PorcupufferEntity> {
    private static final String HELMET = "armorBipedHead";
    public PorcupufferRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new PorcupufferModel());
        this.shadowRadius = 1.8F;

        this.addRenderLayer(new ItemArmorGeoLayer<>(this) {
            @Nullable
            @Override
            protected ItemStack getArmorItemForBone(GeoBone bone, PorcupufferEntity animatable) {
            ...
            @NotNull
            @Override
            protected EquipmentSlot getEquipmentSlotForBone(GeoBone bone, ItemStack stack, PorcupufferEntity animatable) {
                return switch (bone.getName()) {
            ...
            @NotNull
            @Override
            protected ModelPart getModelPartForBone(GeoBone bone, EquipmentSlot slot, ItemStack stack, PorcupufferEntity animatable, HumanoidModel<?> baseModel) {
                return switch (bone.getName()) {
        ...
        this.addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            @Override
            protected ItemStack getStackForBone(GeoBone bone, PorcupufferEntity animatable) {
                return switch (bone.getName()) {
                    case HELMET -> {
            ...
            @Override
            protected ItemDisplayContext getTransformTypeForStack(GeoBone bone, ItemStack stack, PorcupufferEntity animatable) {
                return switch (bone.getName()) {
            ...
            protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, PorcupufferEntity animatable,
                ...
                if (stack.getItem() instanceof BlockItem) {
                    poseStack.scale(3.875F, 3.875F, 3.875F);
