...
public class BlockFacePacket {
    public static final BlockFacePacket INSTANCE = new BlockFacePacket();

    public static BlockFacePacket get() {
        return INSTANCE;
    }

    public void handle(final BlockFacePayload payload, IPayloadContext context) {
                ...
                if (player.containerMenu.containerId == payload.containerId()
                    ...
                    menu.getAccess().execute((level, pos) -> {
                        ...
                        if (blockEntity instanceof BlockSpawnerBlockEntity spawnerBE) {
                            spawnerBE.setBlockFace(payload.blockFace());
